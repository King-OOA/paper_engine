...
\title{A Fast Engine for Multi-string Pattern Matching}


\author{Zhan Peng, Yuping Wang\footnote{Corresponding author}~, Wei Yue}
...
applications such as pattern recognition \cite{Yan2016},
\cite{Xiao2016}, ontology alignments \cite{Xue2015} \cite{Xue2016},
text classification \cite{Tang2015} \cite{Zhang2016}, intrusion detection system
\cite{Kim2015}, etc. In general, it seeks all positions in a text
string that match any of the string patterns in a given pattern
set. An efficient algorithm for MSM only needs to scan the text once
and finds all patterns concurrently. Traditionally, the existing
algorithms include two major phases, i.e. the \textsf{preprocessing
phase} and the \textsf{matching phase}. In the \textsf{preprocessing
phase}, a suitable data structure is constructed to store all the
string patterns, and then the \textsf{matching phase} scans and
compares the given text with the data structure to find all
...
To reduce the memory requirement of \textsf{AC}, Tuck \cite{Tuck2004}
...
proposed by \cite{Bremler2011}. In addition to use a similar path
...
degrades. Recently, Kim \cite{Kim2015} proposed a memory-efficient
algorithm \textsf{Split} for deep packet inspection system, which
consists of multiple finite-state machine.
...
dictionary and 1.32 for the Snort rule set. Moraru \cite{Moraru2013}
...
is a vector of $N$ bits. (If $lsp \leq 5$, set $k = 2$; otherwise, set
$k=3$. $N$ is set to be $|\Sigma|^k$). Let $B_j[i]$,
$0 \leq i \leq N - 1$, $1 \leq j \leq lsp-k+1$, be the $i$-th bit of
the $j$-th query bitmap, and $B_j[i]$ is set to 1 iff there exists a
pattern $p_m$ such that $hash(p_m[j,k]) = i$ ($p_m[j,k]$ is a
$k$-symbol block starting at position $j$ of $p_m$), where $hash$ is a
hash function that maps a $k$-symbol block to an integer in the range
of $[0, N-1]$.
...
Figure \ref{fig:filter} shows an example of the filter, assume that
...
As an example (see Figure \ref{fig:f_match}), consider the query bitmaps
...
in the queue, the whole AMT has been constructed.  It is worth
pointing out that, we can also use a depth-first strategy to build the
AMT branch by branch, but regardless of the strategies used, the final
built AMTs are the same.
...
Given a prefix set, if the length of prefixes (keys) is equal to 1
($lss=1$), i.e. each key in the prefix set is just a single character,
the space-efficient \emph{character map} structure, which was proposed
by Leis \cite{Leis2013}, will be adopted as the tree node. There are
four sub-types of character maps with different capacities for
different number of keys ($ndp$), where $1 \leq ndp \leq 256$.
...
AMT constructed by Algorithm 1. As shown in Figure \ref{fig:merge}, the
...
target key in a node. For instance -- as shown in Figure \ref{fig:split},
...
keys and form another new node as shown in Figure \ref{fig:split}, for
...
compare it with other five algorithms: the \textsf{AC} and \textsf{WM}
algorithms, which are two classic ``baseline'' algorithms for MPM; the
\textsf{Split}, \textsf{MASM} and \textsf{Pre-filter+AC} algorithms,
which are three state-of-the-art MPM algorithms with good efficiency
in practice. All these algorithms are evaluated in two aspects: the
...
Next, the parameter configuration of \textsf{FEMPM} in the experiment
is given as follows: for the String Arrays, if the number of strings
...
String Arrays containing no more than 4 strings, the simple linear
search is faster than binary search due to the complexity of binary
search). For the Hash Tables, the load factor is set to be
\textbf{0.5}, which is a good balance between time and space; the seed
of the hash function is generated randomly in the range of 1~50; the
shift values L and R are set to \textbf{2} and \textbf{6} respectively
(the parameters about the hash function are chosen from
\cite{Ramakrishna1997} which are claimed to be very efficient). Once
the $ndp$ is larger than \textbf{100}, the Hash Table becomes more
efficient than the String Array, thus the Hash Table is built once
$ndp>100$. These choices of parameters have been extensively tested on
a wide range of pattern sets, and are considered to be very efficient
in practice.
...
\begin{figure}[htbp]
  ...
  \caption{Matching times for various lsp.}
  \label{fig:lsp}
\end{figure}
...
the other side, the \textsf{Split} algorithm, which is also based on
DFA, is not effected much by $lsp$, and since it extracts a subset of
unique patterns to build the DFA and uses a bit-split technique, the
memory and time consumption are greatly reduced. It can be seen from
Figure \ref{fig:lsp} that the \textsf{WM} algorithm is affected much
by the changes in $lsp$: it outperforms \textsf{AC} on the pattern
sets whose $lsp$ is larger than 2, but for the pattern sets with small
...
the positions in the text string need to be checked by the HASH table
of \textsf{WM}, and this takes a lot of time. The
\textsf{Pre-filter+AC} algorithm outperforms all other algorithms
except for \textsf{FEMPM} for pattern sets whose $lsp$ is larger than
5. Similarly, this algorithm also uses a skip strategy: once a
position of the text string is filtered out by the Pre-filter, at most
the following $lsp-1$ positions can be skipped over. Consequently, for
pattern sets with small $lsp$, this algorithm can not skip much in the
text string, which will seriously affect its performance. In contrast,
the \textsf{MASM} algorithm performs well for small $lsp$
($lsp \leq 4$). This algorithm first compacts the pattern set by a
prefix tree: if a pattern $A$ is a prefix of another pattern $B$, $A$
can be merged into $B$. Then the algorithm builds a binary search tree
for the compacted pattern set based on lexicographical
order. Comparing with the long patterns, the short ones are more
likely to be prefixes of others, and thus, more likely to be
eliminated by compacting. Thus, pattern sets with small $lsp$ are well
compressed, which will improve both time and space efficiency of
\textsf{MASM} in the matching phase.
...
of corresponding AMTs are shown in table \ref{tab:node types}. The Map
1 and Single String Array types in the table are actually the Map 4
and String Array types which contain only one element,
...
patterns. As before, the size of the text string is fixed to 200MB,
and there are two groups of pattern sets for testing --- the small
group and the large group. The small group contains 9 pattern sets,
and the sizes (number of patterns) of the pattern sets increase from
$1 \times 10^5$ to $9 \times 10^5$ with step size of $10^5$, while the
large group contains 10 pattern sets, and the sizes increase from
$10^6$ to $10^7$ with step size of $10^6$. The range of the pattern
length of each pattern set is fixed to $5 \sim 50$.
...
increases. The \textsf{Split} algorithm performs very well for the
small group of patterns, it is only $10\% \sim 20\%$ slower than our
\textsf{FEMPM}.


\begin{table}[!htp]
  \scriptsize
  \caption{The statistics about the node type in AMTs built for the small group}
  \label{tab:small}
...
(\textsf{MASM}), $191\%$ (\textsf{Split}), $112\%$ (\textsf{FEMPM})
respectively. For every increase of $10^6$ in the number of patterns,
the mean increment in the matching time of the investigated algorithms
are: $23.7s$ (\textsf{WM}), $17.0s$ (\textsf{Pre-filter+AC}), $12.3s$
(\textsf{MASM}), $8.8s$(\textsf{Split}), $3.5s$ (\textsf{FEMPM})
respectively. This indicates that, \textsf{Pre-filter+AC} and
\textsf{WM} perform not well for large pattern sets. This is because,
as the number of patterns grows, the chances for every position of the
text to successfully match a pattern increases, which will reduce the
effect of the filters of these algorithms and results in performance
degradation. The \textsf{MASM} algorithm outperforms
\textsf{Pre-filter+AC} for pattern sets whose size are larger than
$3 \times 10^6$, which is due to its pattern set compression strategy
and the good scalability of its pipelinded binary search tree. The
\textsf{Split} algorithm has a better scalability than others (except
\textsf{FEMPM}), this is because it only extracts a subset from the
whole pattern set, in which a pattern is not a suffix of any other
patterns, to build the DFA.

\begin{table}[!htp]
  \caption{The statistics about the node type in the AMTs built for
    the large group}
  \scriptsize
...
in the number of patterns, only requires 2.8 more seconds to match,
this is mainly due to the good scalability of the AMT which enables
the matching time grows very slow as the number of patterns
explodes. The statistical data about the AMTs for the large group are
shown in Table \ref{tab:large_group}. Every $10^6$ increase in the
number of patterns will increase only about $8 \times 10^5$ tree
nodes.
...
\bibitem{Kim2015} Kim, HyunJin AND Choi, Kang-Il AND Choi,
  Sang-Il. ``A Memory-Efficient Deterministic Finite Automaton-Based
