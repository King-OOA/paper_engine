...
checks that position for real pattern occurrences. Specifically, for
...
immediately forwards to the next position $i+1$ and invokes the
filter.
...
the AMT built in section \ref{subsec:amt}. The verification starts
...
$aa \in root$, and $aa$ is also marked as a pattern end. Therefore the
pattern $T[i,\,2]=aa$ (actually $p_6$ in $P$) is found at position
$i$. Then the verification transfers to $node_1$, which is the child
node of $root.aa$.
...
not a pattern end. Then the verification simply transfers to $node_5$,
...
$i$. Then the verification goes to the child of $node_5.u^5$, i.e.
$node_{10}$.
...
\item \textbf{Map 4} (for $1 \leq ndp \leq 4$): The smallest map type
  ...
  values. Once the target string (character) is found in the key
  array, its child pointer can be located through the same position in
  the pointer array. Figure \ref{fig:character map} (a) shows a Map 4
...
\emph{string array} structure is constructed as a tree node. Similar
to the Map 4 and Map 16 structures, the keys are stored in
...
large number of keys. It is worth noting that, the procedure of
building a hash table is directly based on the suffix set from which
the prefix set is formed rather than the prefix set which is the basis
of building other node structures. In this work, a hash table is an
array of child pointers with each pointer initialized to $NULL$, and
we also utilize a \emph{string hash function} which transforms a
...
corresponding slot is $NULL$, which means the target string fails to
match any key in the hash table, the verification terminates and the
engine moves to the next position; otherwise, the process goes to the
tree node associated with that slot and compares the node with the
...
uniform and universal. The form of the hash function family is shown
in Algorithm \ref{alg:hash fun}.
