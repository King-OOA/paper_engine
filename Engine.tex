...
removed from the keys and form a new single key node. This technique
...
string array type node whose four keys share a common prefix $a^5$, as
shown in Fig . According to Algorithm \ref{alg:string array}, the
...
$a^5$ is split out and form another new node as shown in Fig , for the
same target, only 9 byte comparisons (5 comparisons for $node_1$ and
another 4 comparisons for $node_2$) are needed for
searching. Comparing with only one node, the two nodes can reduce 15
comparisons. Note that, since the prefix is either a character or a
string, the new node is either a Map 4 node or a String Array node.
